...
def _get_trial_length_by_kwarg(events_tsv, start_trial_type, stop_trial_type):
    start_trial_inds = [
        i for i, x in enumerate(events_tsv["trial_type"]) if x == start_trial_type
    ]
    end_trial_inds = [
        i for i, x in enumerate(events_tsv["trial_type"]) if x == stop_trial_type
    ]
...
def read_dataset(bids_fname, bids_root):
    ...
    tmax = _get_trial_length_by_kwarg(
        events_tsv, start_trial_type="show card", stop_trial_type="hide reward"
    )

    # get the epochs
    epochs = mne.Epochs(raw, events, event_id, picks=good_chs, tmax=tmax)
    print(epochs.shape)
