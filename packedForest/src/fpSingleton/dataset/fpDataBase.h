	...
	template <typename T>
		class inputXData
		{
			...
			public:
				...
				inline void prefetchElement(const int &feature,const int &observation, const int &hintNum){
					//_mm_prefetch(&XData[observation][feature], _MM_HINT_T0);
					//TODO why won't the compiler allow this prefetch?
					//__builtin_prefetch(&XData[feature][observation], 0, hintNum);
				}
