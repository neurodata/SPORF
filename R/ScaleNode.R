...
#' Scale new data in current Node to [0,1] based on teh scaling factors
#' from training.
#'
#' Scales the data in the current node to live in [0,1] over the columns
#' with the scaling factors used when training the forest.
#'
#' @param Xnode an n by d numeric matrix or data frame containing the data assigned to the current node. The rows correspond to observations and columns correspond to features.
...
#'
#' @return the scaled data as a matrix
#'
#' @export
#'
#'
PredictScale01 <- function(Xnode, scalingFactors) {
  Xnode <- cbind(Xnode)
  scaledXnode <-
