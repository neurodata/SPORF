  ...
  function(X,
    ...
    if (OOB) {
      if (!forest$params$store.oob) {
        stop(
          "out-of-bag indices for each tree are not stored. RerF must be called with store.oob = TRUE."
        )
      }
    ...
    if (forest$params$rank.transform) {
      if (is.null(Xtrain)) {
        ############ error ############
        stop(
          "The model was trained on rank-transformed data. Xtrain must be provided in order to embed Xtest into the rank space"
        )
      } else {
    ...
    if (OOB) {
      CompPredictCaller <- function(tree, ...)
        RunOOB(X = X, tree = tree)
    } else {
      CompPredictCaller <-
        function(tree, ...)
          RunPredict(X = X, tree = tree, task = task)
    }
    ...
    if (num.cores != 1L) {
      ...
      gc()
      if ((utils::object.size(forest) > 2e+09) |
          (utils::object.size(X) > 2e+09) |
          .Platform$OS.type == "windows") {
        cl <- parallel::makeCluster(spec = num.cores, type = "PSOCK")
        parallel::clusterExport(
          cl = cl,
          varlist = c("X", "RunPredict"),
          envir = environment()
        )
        Yhats <-
          parallel::parLapply(cl = cl, forest$trees, fun = CompPredictCaller)
      } else {
    ...
    if (!aggregate.output) {
      if (output.scores) {
        ...
        predictions <-
          matrix(labels[unlist(predictions)], ncol = f_size)
