...
testSets <- c(
  "Orthant_test.csv", "Sparse_parity_test.csv", "Trunk_test.csv", "mnist_test.csv",
...
testDS <- function(trainSet, testSet, numTests, numTrees, cores, p1) {
  ...
  for (i in 1:numTests) {
    ...
    if (length(forest$trees) != numTrees) {
      print(paste(
        "rfr failed to make desired number of trees.  ", length(forest$trees),
  ...
  print(paste(
    "Node Size, pval(2sided)= ", wt, ", ratio(C/B)= ", NodeSizeRatio,
    ", pass=", wt > 0.001
  ))
  if (!is.nan(wt)) {
    if (wt < 0.001) {
  ...
  print(paste(
    "OOBerror, pval(2sided)= ", wt, ", ratio(B/C)= ", OOBerrorRatio,
    ", pass=", wt > 0.001
  ))
  if (!is.nan(wt)) {
    if (wt < 0.001) {
  ...
  print(paste(
    "Error, pval(2sided)= ", wt, ", ratio(B/C)= ", errorRatio, ", pass=",
    wt > 0.001
  ))
  if (!is.nan(wt)) {
    if (wt < 0.001) {
  ...
  print(paste(
    "Train Time, pval(l)= ", wt, ", ratio(B/C)= ", ptmtrainRatio, ", pass=",
    wt > 0.001
  ))
  if (!is.nan(wt)) {
    if (wt < 0.001) {
  ...
  print(paste(
    "Test Time, pval(l)= ", wt, ", ratio(B/C)= ", ptmtestRatio, ", pass=",
    wt > 0.001
  ))
  if (!is.nan(wt)) {
    if (wt < 0.001) {
  ...
  print(paste(
    "OOB Time, pval(l)= ", wt, ", ratio(B/C)= ", ptmOOBRatio, ", pass=",
    wt > 0.001
  ))
  if (!is.nan(wt)) {
    if (wt < 0.001) {
...
for (z in 1:(2 * length(trainSets) - skipMNIST)) {
  gc()
  lh[z] <- testDS(
    trainSets[ceiling(z / 2)], testSets[ceiling(z / 2)], numTests[z],
