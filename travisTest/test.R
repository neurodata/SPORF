...
pvals <- c(0.5, 2, 0.5, 2, 0.5, 2, 0.5, 1.35)
...
testDS <- function(trainSet, testSet, numTests, numTrees, cores, p1) {
    ...
    for (i in 1:numTests) {
        ...
        if (length(forest$trees) != numTrees) {
            print(paste("rfr failed to make desired number of trees.  ", length(forest$trees), 
                " trees made.  ", numTrees, " trees requested."))
            pass <- FALSE
        }
        temp_size <- 0
    ...
    print(paste("Node Size, pval(2sided)= ", wt, ", ratio(C/B)= ", NodeSizeRatio, 
        ", pass=", wt > 0.001))
    if (!is.nan(wt)) {
        if (wt < 0.001) {
            pass <- FALSE
    ...
    print(paste("OOBerror, pval(2sided)= ", wt, ", ratio(B/C)= ", OOBerrorRatio, 
        ", pass=", wt > 0.001))
    if (!is.nan(wt)) {
        if (wt < 0.001) {
            pass <- FALSE
    ...
    print(paste("Error, pval(2sided)= ", wt, ", ratio(B/C)= ", errorRatio, ", pass=", 
        wt > 0.001))
    if (!is.nan(wt)) {
        if (wt < 0.001) {
            pass <- FALSE
    ...
    print(paste("Train Time, pval(l)= ", wt, ", ratio(B/C)= ", ptmtrainRatio, ", pass=", 
        wt > 0.001))
    if (!is.nan(wt)) {
        if (wt < 0.001) {
            pass <- FALSE
    ...
    print(paste("Test Time, pval(l)= ", wt, ", ratio(B/C)= ", ptmtestRatio, ", pass=", 
        wt > 0.001))
    if (!is.nan(wt)) {
        if (wt < 0.001) {
            pass <- FALSE
    ...
    print(paste("OOB Time, pval(l)= ", wt, ", ratio(B/C)= ", ptmOOBRatio, ", pass=", 
        wt > 0.001))
    if (!is.nan(wt)) {
        if (wt < 0.001) {
            pass <- FALSE
...
for (z in 1:(2 * length(trainSets) - skipMNIST)) {
    gc()
    lh[z] <- testDS(trainSets[ceiling(z/2)], testSets[ceiling(z/2)], numTests[z], 
        numTrees[z], cores[z], pvals[z])
    if (lh[z] == FALSE) {
        pass <- FALSE
