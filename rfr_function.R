runrfr <- function(X, Y, MinParent, trees, MaxDepth, bagging, replacement, stratify, Cindex, classCt, FUN, options, COOB, CNS, Progress){
  ...
  treeMap[CurrentNode] <- currIN <- currIN + 1 
      ...
      if (options[[3]] != "frc" && options[[3]] != "continuous") {
        matA[[currIN]] <- as.integer(t(sparseM[lrows, c(1,3)]))
      } else {
        matA[[currIN]] <- t(sparseM[lrows, c(1,3)])
      }
    ...
    if (CNS) {
      if(bagging!=0 && COOB){
        forest[[treeX]] <- list("treeMap"=treeMap[1:NextUnusedNode-1L], "CutPoint"=CutPoint[1:currIN],"ClassProb"=ClassProb[1L:currLN,,drop=FALSE],"Children"=Children[1L:currIN], "matA"=matA[1L:currIN], "ind"=which(!(1:w %in% ind)), "NdSize" = NdSize[1L:(NextUnusedNode-1L)])
      }else{
        forest[[treeX]] <- list("treeMap"=treeMap[1:NextUnusedNode-1L], "CutPoint"=CutPoint[1:currIN],"ClassProb"=ClassProb[1L:currLN,,drop=FALSE],"Children"=Children[1L:currIN], "matA"=matA[1L:currIN], "NdSize" = NdSize[1L:(NextUnusedNode-1L)])
      }
    }
    else {
      if(bagging!=0 && COOB){
        forest[[treeX]] <- list("treeMap"=treeMap[1:NextUnusedNode-1L], "CutPoint"=CutPoint[1:currIN],"ClassProb"=ClassProb[1L:currLN,,drop=FALSE],"Children"=Children[1L:currIN], "matA"=matA[1L:currIN], "ind"=which(!(1:w %in% ind)))
      }else{
        forest[[treeX]] <- list("treeMap"=treeMap[1:NextUnusedNode-1L], "CutPoint"=CutPoint[1:currIN],"ClassProb"=ClassProb[1L:currLN,,drop=FALSE],"Children"=Children[1L:currIN], "matA"=matA[1L:currIN])
      }
    }
    if(Progress){
      cat("|")
...
RunErr <- function(X,Y,Forest, index=0L, chunk_size=0L){
  ...
  for(i in 1:n){
    ...
    for(j in 1:forestSize){
      Tree <- Forest[[j]]
      currentNode <- 1L
      while(Tree$treeMap[currentNode]>0L){
        s<-length(Tree$matA[[Tree$treeMap[currentNode]]])/2
        rotX<-sum(Tree$matA[[Tree$treeMap[currentNode]]][(1:s)*2]*X[i,Tree$matA[[Tree$treeMap[currentNode]]][(1:s)*2-1]])
        if(rotX<=Tree$CutPoint[Tree$treeMap[currentNode]]){
          currentNode <- Tree$Children[Tree$treeMap[currentNode]]
        }else{
          currentNode <- Tree$Children[Tree$treeMap[currentNode]]+1
        }
      }
      classProb <- classProb + Tree$ClassProb[Tree$treeMap[currentNode]*-1,]
    }
  ...
  for (j in 1:(forestSize)) {
    if (!is.null(Forest[[j]]$rotmat)) {
      ...
      for (i in Forest[[j]]$ind) {
        currentNode <- 1L
        while (Forest[[j]]$treeMap[currentNode]>0L) {
          s<-length(Forest[[j]]$matA[[Forest[[j]]$treeMap[currentNode]]])/2
          rotX <-sum(Forest[[j]]$matA[[Forest[[j]]$treeMap[currentNode]]][(1:s)*2]*Xtree[i,Forest[[j]]$matA[[Forest[[j]]$treeMap[currentNode]]][(1:s)*2-1]])
          if (rotX<=Forest[[j]]$CutPoint[Forest[[j]]$treeMap[currentNode]]) {
            currentNode <- Forest[[j]]$Children[Forest[[j]]$treeMap[currentNode]]
          } else {
            currentNode <- Forest[[j]]$Children[Forest[[j]]$treeMap[currentNode]]+1
          }
        }
        classProb <- Forest[[j]]$ClassProb[Forest[[j]]$treeMap[currentNode]*-1,]
      ...
      for (i in Forest[[j]]$ind) {
        ...
        while (Forest[[j]]$treeMap[currentNode]>0L) {
          s<-length(Forest[[j]]$matA[[Forest[[j]]$treeMap[currentNode]]])/2
          rotX <-sum(Forest[[j]]$matA[[Forest[[j]]$treeMap[currentNode]]][(1:s)*2]*X[i,Forest[[j]]$matA[[Forest[[j]]$treeMap[currentNode]]][(1:s)*2-1]])
          if (rotX<=Forest[[j]]$CutPoint[Forest[[j]]$treeMap[currentNode]]) {
            currentNode <- Forest[[j]]$Children[Forest[[j]]$treeMap[currentNode]]
          } else {
            currentNode <- Forest[[j]]$Children[Forest[[j]]$treeMap[currentNode]]+1
...
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
rfr <- function(X, Y, MinParent=6L, trees=100L, MaxDepth=0L, bagging = .2, replacement=TRUE, stratify=FALSE, FUN=makeA, options=c(ncol(X), round(ncol(X)^.5),1L, 1/ncol(X)), rank.transform = FALSE, COOB=FALSE, CNS=FALSE, Progress=FALSE, NumCores=0L, seed = 1L){
