...
improv8::improv8(const std::string& forestFileName){
	std::ifstream infile (forestFileName);
...
improv8::improv8(const std::string& forestCSVFileName, int source, const std::string& travsCSVFileName, int numberBins, int depthIntertwined){
	if(source == 1){
		inferenceSamples observations(travsCSVFileName);
...
void improv8::writeForest(const std::string& forestFileName){
	std::ofstream outfile (forestFileName);

	outfile << numOfBins << std::endl;
	for(int i = 0 ; i < numOfBins; i++){

		outfile << forestRoots[i]->numOfTreesInBin << std::endl;
		outfile << forestRoots[i]->depth << std::endl;
		outfile << forestRoots[i]->numOfClasses << std::endl;
		outfile << forestRoots[i]->numOfNodes << std::endl;

		for(int j = 0; j < (forestRoots[i]->numOfNodes+forestRoots[i]->numOfClasses); j++){

			outfile	<< forestRoots[i]->bin[j].left << std::endl;
			outfile << forestRoots[i]->bin[j].feature << std::endl;
			outfile << forestRoots[i]->bin[j].cutValue << std::endl;
			outfile << forestRoots[i]->bin[j].right << std::endl;
		}
	}
	outfile.close();
}
