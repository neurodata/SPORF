rfr <- function(X, Y, MinParent=6, trees=100, MaxDepth=0, bagging = 0, FUN=makeA, options=ncol(X)){
    ...
    for(treeX in 1:trees){
        ...
        forest[[treeX]] <- list("CutVar"=CutVar[1:NextUnusedNode],"CutPoint"=CutPoint[1:NextUnusedNode],"ClassProb"=ClassProb[1:NextUnusedNode,],"Children"=Children[1:NextUnusedNode,], "matA"=matA[1:NextUnusedNode])
