...
nnzPts <- which(sM$similarityMatrix[, 499:501] != 0 & sM$similarityMatrix[, 499:501] !=
  1)
...
p <- ggplot(aes(x = Nearness, y = Distance, color = Sample), data = ssd) + geom_point() +
  labs(title = "Distance vs Similarity of points 499-501 to all other points\n10-D Line, n=1000, depth=8, trees=100\nThree Samples (0 Similarity omitted)") +
  geom_jitter()
p <- p + scale_y_log10()
print(p)
