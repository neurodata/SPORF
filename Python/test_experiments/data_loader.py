...
def load_sbms_Xy(
    ...
    if not return_meta:
        ...
        return X, y, (sum(communities1), sum(communities1))


## Graph signal
def load_graphsignal_Xy(n1, n2, ns, seed=None, return_meta=True):
    np.random.seed(seed)
    X = []
    y = []
    G = er_np(10, 0.3)
...
def load_correlated_ts_Xy(n1, n2, cov1, cov2, tsteps, ns=None, seed=None):
    def get_X(cov):
...
## Impulse Response
def load_impulse_Xy(n1, n2, ns=None, seed=None, return_meta=True):
    def samp(n, cls=0, separate=False):
        ...
        M1 = 100 - M0
        func = lambda x: [
            np.exp(-0.1 * (j - M0)) if j >= M0 else 0.0 for j in x
    ...
    if not return_meta:
        ...
        return X, y, (1, int(len(X[0])))


## Load HVBar
def load_hvbar_Xy(n1, n2, ns=None, seed=None, return_meta=True):
...
def load_cifar10(batches=[1, 2, 3, 4, 5]):
    data_dir = Path("/mnt/ssd3/ronan/data/cifar-10-batches-py")
