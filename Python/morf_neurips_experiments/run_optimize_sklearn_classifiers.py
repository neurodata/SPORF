...
class Experiment:
    def __init__(self, experiment):
        ...
        self.param_dict['max_features'] = np.sqrt(self.imageHeight * self.imageWidth) * \
            (self.patch_height_max - self.patch_height_min + 1) * \
            (self.patch_width_max - self.patch_width_min + 1)
...
## Run Optimization
    ...
    def eval_classifiers(
        ...
        classifiers = [
            Pipeline([('scaler', StandardScaler()), (name, clf)]) for name,clf in zip(classifiers, names)
        ]
...
### Main
# Ex. python run_optimize_sklearn_classifiers.py --eval --experiments impulse --clf MORF --tag response --save
