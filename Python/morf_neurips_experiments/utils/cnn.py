...
class ImpulseModel(nn.Module):
    def __init__(self, h, w):
        ...
        linear1_in_size = 64
        linear1_in_size *= h
        linear1_in_size *= ((w - kernel_size + 2 * padding + 1) / 2 - \
            kernel_size + 2 * padding + 1) / 2
        self.layer1 = nn.Sequential(
            nn.Conv1d(1, 32, kernel_size=kernel_size, stride=1, padding=padding),
            nn.ReLU(),
            nn.MaxPool1d(kernel_size=2, stride=2))
        self.layer2 = nn.Sequential(
            nn.Conv1d(32, 64, kernel_size=kernel_size, stride=1, padding=padding),
            nn.ReLU(),
            nn.MaxPool1d(kernel_size=2, stride=2))
        self.drop_out = nn.Dropout(p=0.5)
        self.fc1 = nn.Linear(linear1_in_size, 500)
...
class HVBarModel(nn.Module):
    def __init__(self, h, w):
        ...
        padding = kernel_size - 1
        linear1_in_size = 64
        linear1_in_size *= ((h - kernel_size + 2 * padding + 1) / 2 - \
                kernel_size + 2 * padding + 1) / 2
        linear1_in_size *= ((w - kernel_size + 2 * padding + 1) / 2 - \
                kernel_size + 2 * padding + 1) / 2
        self.layer1 = nn.Sequential(
        ...
        self.layer2 = nn.Sequential(
            ...
            nn.MaxPool2d(kernel_size=2, stride=2))
        self.drop_out = nn.Dropout(p=0.5)
        self.fc1 = nn.Linear(linear1_in_size, 200)
