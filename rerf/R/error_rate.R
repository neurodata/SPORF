...
#' 
#' @examples
#' error_rate(as.matrix(iris[,1:4]), as.numeric(iris[,5]), Forest)
#'
#' @export
  ...
  if(NumCores!=1){
    if(requireNamespace(parallel)){
      ...
      if (nrow(X)%%NumCores==0){
        ...
        gc()
        total_misclassified <- sum(as.numeric(parallel::mclapply(1L:NumCores,comp_err_caller, mc.cores=NumCores)))
      }else{
        if(nrow(X) > NumCores){
        ...
        gc()
        total_misclassified <- total_misclassified + sum(as.numeric(parallel::mclapply(1L:(nrow(X)%%NumCores), comp_err_caller, mc.cores=(nrow(X)%%NumCores))))
      }
    }else{
